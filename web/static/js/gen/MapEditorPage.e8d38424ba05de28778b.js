...
class MapEditorPage {
    constructor() {
        ...
        this.wasmWorld = null;
        this.wasmViewState = null;
        this.wasmCanvasRenderer = null;
    ...
    initializeWasm() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const worldResult = window.worldCreate(2, null, 12345);
                if (!worldResult.success) {
                    throw new Error(`World creation failed: ${worldResult.error}`);
                }
                this.wasmWorld = worldResult.data;
                const viewStateResult = window.viewStateCreate();
                ...
                const rendererResult = window.canvasRendererCreate();
                ...
                const assetsResult = window.loadEmbeddedAssets();
                ...
                const testResult = window.testEmbeddedAssets();
    ...
    createNewMap(width, height) {
        ...
        if (this.wasmInitialized) {
            try {
                const result = window.editorSetMapSize(height, width);
                if (result.success) {
    ...
    setBrushTerrain(terrain) {
        ...
        if (this.wasmInitialized) {
            try {
                const result = window.editorSetBrushTerrain(terrain);
                if (!result.success) {
    ...
    setBrushSize(size) {
        ...
        if (this.wasmInitialized) {
            try {
                const result = window.editorSetBrushSize(size);
                if (!result.success) {
    ...
    paintHexAtCoords(row, col) {
        ...
        try {
            const result = window.editorPaintTerrain(row, col);
            if (result.success) {
                if (this.mapData) {
                    const key = `${row},${col}`;
    ...
    addRow(side) {
        if (!this.mapData || !this.wasmInitialized)
            return;
        this.logToConsole(`Adding row to ${side}`);
        const newHeight = this.mapData.height + 1;
        try {
            const result = window.editorSetMapSize(newHeight, this.mapData.width);
            if (result.success) {
    ...
    removeRow(side) {
        if (!this.mapData || this.mapData.height <= 1 || !this.wasmInitialized)
            return;
    ...
    addColumn(side) {
        ...
        try {
            const result = window.editorSetMapSize(this.mapData.height, newWidth);
            if (result.success) {
    ...
    removeColumn(side) {
        if (!this.mapData || this.mapData.width <= 1 || !this.wasmInitialized)
            return;
...
//# sourceMappingURL=MapEditorPage.e8d38424ba05de28778b.js.map