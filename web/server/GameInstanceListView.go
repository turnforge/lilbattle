...
type GameInstanceListView struct {
	GameInstances  []*protos.GameInstance
	Paginator Paginator
}

func (g *GameInstanceListView) Copy() View { return &GameInstanceListView{} }

func (p *GameInstanceListView) Load(r *http.Request, w http.ResponseWriter, vc *ViewContext) (err error, finished bool) {
	...
	client, _ := vc.ClientMgr.GetGameInstancesSvcClient()

	req := protos.ListGameInstancesRequest{
	...
	resp, err := client.ListGameInstances(context.Background(), &req)
	if err != nil {
		log.Println("error getting notations: ", err)
		return err, false
	}
	log.Println("Found GameInstances: ", resp.Items)
	p.GameInstances = resp.Items
