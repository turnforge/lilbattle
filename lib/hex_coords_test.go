...
func TestAxialCoordinates(t *testing.T) {
	...
	coord := NewAxialCoord(1, 2)
	if coord.Q != 1 || coord.R != 2 || coord.S() != -3 {
		t.Errorf("NewAxialCoord(1, 2) = %+v, expected Q=1, R=2, S=-3", coord)
	}
