...
func ParsePositionOrUnit(game *weewar.Game, input string) (*ParseTarget, error) {
	...
	if strings.HasPrefix(strings.ToLower(input), "r") {
		return parseRowColCoordinate(input[1:])
	}
	
	// Check if it's a coordinate (contains comma)
	if strings.Contains(input, ",") {
		return parseQRCoordinate(input)
	}
	
	// Try to parse as unit ID
	if game != nil {
		if target, err := parseUnitID(game, input); err == nil {
			return target, nil
		}
	}
	
	return nil, fmt.Errorf("invalid format: %s (expected unit ID like A1 or coordinate like 3,4 or r4,5)", input)
}
